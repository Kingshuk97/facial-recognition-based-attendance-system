...
#window.bind("<Return>", self.clicked)
            # bind the action of the left button of your mouse to the button assuming your primary click button is the left one.
#button.bind("<Button-1>", self.clicked)

            # call the mainloop function so the window won't fade after the first execution
window.mainloop()
